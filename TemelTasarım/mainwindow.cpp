...
void MainWindow::randevuGoster()
{
    ...
    QStringListModel *model = new QStringListModel(randevuListe,this);
...
void MainWindow::randevuSil()
{
    ...
    QSqlQuery query;
    query.prepare("DELETE FROM randevular WHERE ad = ? AND tc = ? AND tarih = ? AND saat = ? AND doktor = ?"); // Bu kısmı id değerine göre silecek şekilde ayarla.
    query.addBindValue(ad);
    query.addBindValue(tc);
    query.addBindValue(tarih);
    query.addBindValue(saat);
